...
- [ ] 完成真实登录闭环：当前 `pages/login/login` 仍是前端模拟登录，尚未切换到正式的微信身份接入方案
- [ ] 联调并验证 `login` 云函数、`users` 集合与用户资料页之间的完整链路
...
目前项目已经**适合继续做联调、验收和上线准备**，但还**不能视为正式上线完成**。最关键的未完成项是：

- 登录体系仍未从“演示页”升级为“正式身份闭环”
