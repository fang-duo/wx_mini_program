...
### 第十阶段：真实微信登录、隐私流程与 AI 提示优化 (2026-05-25)

#### 问题背景

在最近一轮真机和界面检查中，暴露出几个直接影响体验和提审判断的问题：

- 个人中心“微信授权登录”仍依赖旧的 `wx.getUserProfile` 获取资料流程，真机上容易出现“授权失败”
- 已登录状态和“是否拿到昵称”耦合过深，导致真实身份已获取时，页面仍可能被判断为未登录
- 多个弹窗按钮文字没有在按钮内部居中，影响观感
- AI 页面缺少明确的医疗免责声明
- AI 顶部提醒只有“以后不再提醒”，缺少“稍后提醒”这类更温和的交互
- 首页隐私协议弹窗只在首次处理后出现一次，后续刷新不会再次触发，用户容易误以为流程失效
...
1. **真实登录以 `openid` 为准**
   ...
   - 登录必须使用真实微信身份方案
   - 以 `wx.cloud.callFunction({ name: 'login' })` 获取 `openid` 作为登录成功依据
   - 头像、昵称属于资料完善，不再作为登录前提

2. **隐私协议状态持久化**

   - 首页隐私弹窗的处理结果继续保存在本地缓存 `privacy_state`
   - 重新编译、刷新页面不会自动清空该状态
   - 只有清缓存、删 `privacy_state`，或修改协议版本号时，才会重新触发首个隐私弹窗

3. **AI 提醒交互分级**

   - “以后不再提醒”表示关闭顶部提示与相关提醒入口
   - “稍后提醒”仅隐藏当前会话顶部提示，不关闭离开页面前的提醒能力
...
- 登录弹层改为直接调用 `login` 云函数
- 成功获取 `openid` 后即视为登录成功
- 同步保存全局登录态、本地缓存和云端用户记录
- 登录失败提示从“授权失败”收口到真实云函数链路

##### 2. 登录状态判断逻辑修复

- 修改 `utils/access.js`
- 将“是否登录”的判断从“必须存在昵称”扩展为：
  - 已记录 `loggedIn`
  - 或已存在 `openid`
  - 或已有有效用户资料
- 避免真实登录成功后被误判成未登录
...
- 去掉对旧 `wx.getUserProfile` 登录主流程的依赖
- 统一改为通过云函数获取真实身份

##### 4. 按钮居中样式统一修复

- 修改 `pages/profile/profile.wxss`
...
- 修改 `pages/ai/ai.js`
- 新增提示文案：
  - “AI 回答问题仅供参考，不替代专业医疗建议。”
- 顶部提醒区新增“稍后提醒”按钮
- “稍后提醒”仅隐藏当前顶部提示，不关闭离页提醒

#### 本阶段涉及文件

- `pages/profile/profile.js` - 真实微信登录逻辑
- `pages/profile/profile.wxss` - 登录弹层与未登录卡片按钮居中
- `pages/login/login.js` - 独立登录页切换到真实登录
- `utils/access.js` - 登录态判断修复
- `pages/index/index.wxss` - 隐私弹窗按钮居中
- `pages/ai/ai.wxml` - 新增免责声明与“稍后提醒”入口
- `pages/ai/ai.wxss` - AI 提示区样式
- `pages/ai/ai.js` - “稍后提醒”交互逻辑

#### 当前结果

- 项目登录体系已进一步从“可演示”收口到“真实微信身份登录”
- AI 页面合规提示更加清晰
- 顶部提醒交互更完整
- 隐私弹窗的“只弹一次”行为已确认是缓存设计导致，而不是失效
...
14. ✅ 个人中心与登录页统一为真实微信身份登录方案
15. ✅ 首页隐私弹窗、登录弹层按钮文本居中修复
16. ✅ AI 页面新增医疗免责声明与“稍后提醒”交互
